...
事前知識は、追加検索とは別に設定コンテキストとして質問設計・質問生成・回答解釈・非同期整理へ提供する。GPT-Liveの初期会話コンテキストも対象とする。利用・確認・出典のルールは[プロダクト仕様](../../spec.md)7.3.3を正本とし、実装は`services/prior_knowledge.py`の共通経路を利用する。
