...
ドキュメントは、管理者が内容を確認したうえで登録し、取り込み状態を管理する。
...
文書本文は認証済みのBackend APIからアップロードする。Backendはファイル形式とサイズを検証し、本文を抽出してチャンク化した後、設定された文書検索Repositoryへ保存する。文書メタデータ、取り込み状態、Knowledge・Recordなどの業務データはPostgreSQLを正本とする。
...
| 事前知識 | 参照文書の追加、本文の表示・削除、取り込み状態、チャンク数、取り込みエラー |

「ナレッジ情報」は3つのタブのいずれにも含めず、タブの上に配置する。設定画面の保存操作は、ナレッジ情報、質問項目、実行設定をまとめて保存する。事前知識の文書追加・本文表示・削除は、それぞれの操作時に保存する。タブごとに別の設定保存状態を持たせてはいけない。
...
文書由来の候補は、利用者が明示的に承認するまで正式回答へ保存してはならない。候補を持つ項目は`AWAITING_CONFIRMATION`として保持し、承認時だけ`CONFIRMED`へ移し、訂正時は文書候補を破棄して利用者発話の値へ置き換える。根拠がない、対象項目との意味的一致が弱い、または文書が`indexed`でない場合は候補を作らず、通常の質問を行う。`retrievalPolicy=never`の項目はこの候補化の対象外とする。
