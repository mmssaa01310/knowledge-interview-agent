...
「ヘルプ」は操作マニュアルを別タブで開く。「操作ガイド」は現在の画面上で、`data-guide`等の安定した対象を順番にハイライトし、戻る・次へ・終了で操作場所を案内する。ガイド定義は画面Componentから分離し、画面幅に応じて表示されるDrawer内の対象も同じ定義で扱う。ガイドは初回の画面案内、ナレッジ作成・インタビュー設定・質問設定、インタビュー実施、記録確認・修正、ナレッジ設定、管理者分析の単位に分ける。管理者分析ガイドは`admin`と`knowledge_manager`だけに表示する。確認が必要なナレッジ項目は既存のインタビュー状態を正本として動的に対象化し、`CONFIRMED`済みの項目を不要に案内しない。ナレッジ作成後は、ユーザーが選択した場合だけ確認ガイドを開始する非ブロッキング案内を表示する。
