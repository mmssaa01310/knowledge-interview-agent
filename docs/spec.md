...
事前知識のタイトル、種別（既知情報／専門用語）、本文、本文形式（テキスト／Markdown）を認証済みBackend APIへ送信する。Backendは空本文・サイズ上限・権限を検証し、本文をチャンク化して既存の文書検索Repositoryへ保存する。メタデータ、改訂識別子、登録・更新ユーザーID、反映状態、Knowledge・Recordなどの業務データはPostgreSQLを正本とする。新しいDB・検索サービスは追加しない。
...
入力欄はタイトル、種別、本文形式、本文で構成する。既知情報には業務の前提、既存の仕組み、既に分かっている事実を記載する。専門用語には名称、略称・別名、意味、適用範囲を本文として記載できる。Markdownは見出しや箇条書きを含めて保存し、任意HTMLやスクリプトを実行しない。ファイル選択・ドラッグ＆ドロップは提供しない。
...
本節は変更後の仕様である。現行実装との差分と実装順序は[実装計画](plans/interview-prior-knowledge-text.md)を参照する。
