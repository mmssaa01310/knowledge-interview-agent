...
`MUSUBI`（ムスビ）は「結び」を表す。現場の人と対話し、そこで得た知恵を確認・共有できる知識へつなぐという意味である。
...
* ドキュメント取り込み状態管理
* ナレッジのタグ付けとタグ別グループ化
* 企業IdPによるログイン（候補: Microsoft Entra ID。IdPは未確定）
* ログインユーザーに紐づいたデータ保存
* PostgreSQLを正本とし、文書検索はPostgreSQLまたはElastic Cloudへ切り替え可能な検索・保存
* 将来の外部DB送信
...
画面幅が狭い場合は、左列、会話、処理の流れの順に縦へ並べる。ただし、記録のインタビュー実行中は会話と回答Composerを優先する。`1199px`以下の実行画面ではアプリの利用可能な高さをWorkspaceに割り当て、会話履歴だけを内部スクロールさせる。画面全体の通常スクロールは、開始前、設定、記録確認など実行中以外の画面で許可する。画面幅が広い場合に進捗項目が画面高さを超えるときは、左列内でスクロールできるようにする。

右列の会話には、次の質問対象を1件だけ「いま確認していること」として表示する。質問対象がない場合は、インタビュー開始前、回答整理中、完了後の状態に応じた案内を表示する。

会話中のユーザー発言には表示名「あなた」を表示しない。ユーザー側の吹き出しの配置・配色で発言者を区別する。AI発言には文字ラベルを表示し、キャラクター画像やマスコットは表示しない。処理中やエラーは、状態テキストとラベルの色で区別する。
...
事前知識は、管理者が内容を確認したうえで、テキストまたはMarkdownの本文を直接入力して登録する。ファイルのアップロード・本文抽出を入力経路としない。PDF / DOCX / XLSX / PPTX / CSVへの対応は不要とし、TXT / Markdownもファイルではなく入力・貼り付けで登録する。利用時の振る舞いは7.3.3を正本とする。
...
上記は直接入力した本文の検索反映状態であり、ファイル抽出工程は設けない。旧文書の状態は移行互換として扱い、新規登録に旧工程を要求しない。本文の保存成功と検索への反映完了を区別し、失敗時は再反映できるようにする。

### 4.2 本文取り込みと検索Repository

事前知識のタイトル、種別（既知情報／専門用語）、本文を認証済みBackend APIへ送信する。Backendは空本文・サイズ上限・権限を検証し、本文を正規化してからチャンク化し、既存の文書検索Repositoryへ保存する。本文形式はBackendが自動判定する。メタデータ、改訂識別子、登録・更新ユーザーID、反映状態、Knowledge・Recordなどの業務データはPostgreSQLを正本とする。新しいDB・検索サービスは追加しない。

更新・削除後は古い本文・チャンクを新しい質問の根拠として使用してはならない。検索結果の改訂と有効性をBackendで検証する。既存記録に保存された出典は履歴として保持するが、現行の事前知識とは区別する。

文書本文・チャンクのRepositoryは`DOCUMENT_KNOWLEDGE_BACKEND=postgres`（既定）または`elasticsearch`で切り替える。`elasticsearch`ではElastic CloudのCloud IDとAPI Keyを使用し、`documents`および`document_chunks`インデックスへ保存する。検索時は同じテナント、同じKnowledge、`indexed`または`completed`の文書だけを対象にする。Backendを切り替えた場合、既存文書は対象Repositoryへ再インデックスしてから参照する。
...
DB受付済み字幕の整理・保存はページ離脱後もBackendで継続し、未処理分は再起動後も再試行する。GPT-Live経路でも共通のProfile別完了条件と最後の追加事項への回答を検証して完了を保存し、画面の聞き取り状態を終了する。未確定の回答候補は処理中と混同せず「補足・確認待ち」と表示する。
...
| `interviewer` | インタビュー対象者 | 有効なナレッジから自分の記録を開始・回答し、レビューを依頼する。 |
...
全ロールにナレッジ一覧を表示する。`admin`と`knowledge_manager`には管理可能なナレッジ、`interviewer`には同一テナントの有効なナレッジ、`viewer`には閲覧を許可された記録が存在するナレッジだけを表示する。記録は必ず選択したナレッジ内の「記録」から扱う。

左サイドバーは、全ロールで次の内容を1列で表示する。

* 見出し「ナレッジ」、権限範囲内のナレッジ一覧。`admin`と`knowledge_manager`だけにナレッジ作成ボタンを表示する。
* 下部にログインユーザーの表示名とロールを示すユーザーメニューを表示し、表示言語、表示テーマ、ヘルプ、操作ガイド、ログアウトをメニュー内にまとめる。表示テーマは既定をライトとし、ユーザーがライト／ダークを切り替えられる。選択はブラウザに保存し、表示テーマの変更で業務データ、認証、AIの振る舞いを変更してはいけない。`admin`だけはユーザーメニュー内から「システム設定」を開ける。開発用ユーザー切替は開発環境のユーザーメニュー内だけに表示する。

「ヘルプ」は操作マニュアルを別タブで開く。「操作ガイド」は現在の画面上で、`data-guide`等の安定した対象を順番にハイライトし、戻る・次へ・終了で操作場所を案内する。ガイド定義は画面Componentから分離し、画面幅に応じて表示されるDrawer内の対象も同じ定義で扱う。ガイドは利用者ロール、現在の画面、選択中の記録状態からおすすめを選ぶ。初回の画面案内、ナレッジ作成、インタビュー設定、質問設定、インタビュー実施、記録確認・修正、閲覧、管理者向けの全体分析・優先対応・学習分析を、それぞれ短い目的単位に分ける。`interviewer`には回答と差し戻し修正、`viewer`には閲覧、`admin`と`knowledge_manager`には作成・レビュー・分析を表示する。確認が必要なナレッジ項目は既存のインタビュー状態を正本として動的に対象化し、`CONFIRMED`済みの項目を不要に案内しない。ガイド中に利用者の操作が必要な場合は、その結果が画面に現れたことを確認してから次のStepへ進める。ナレッジ作成後は、ユーザーが選択した場合だけ設定ガイドを開始する非ブロッキング案内を表示する。
...
サイドバーは開閉できる。閉じた状態では開閉ボタンだけを表示し、メイン画面を広げる。`1200px`以上では既存の左サイドバーを維持する。`1199px`以下では左サイドバーをDrawerとして表示し、メイン画面上部の開閉ボタンから操作する。ナビゲーションのデータと操作は画面幅によらず共通とし、User-Agentや端末種別で表示を分岐してはならない。

ナレッジ一覧は、利用者が表示順を設定していない場合、`Knowledge.createdAt`の昇順で表示する。同一の作成日時の場合は`Knowledge.id`の昇順とする。利用者は左サイドバーのナレッジごとのピンアイコンをクリックしてピン留めを切り替え、ドラッグハンドルで同じピン留め状態の範囲内にあるナレッジを並べ替えできる。ピン留めしたナレッジはサイドバー上部の「ピン留め」グループへまとめ、元のタググループには重複表示しない。タググループ内の並べ替えは、現在表示しているグループ内の項目を基準とする。これらは認証ユーザーとテナント単位のブラウザ表示設定として保存し、ナレッジの認可範囲、業務データ、他の利用者の表示順を変更してはならない。ナレッジを選択しても、表示順を変更してはいけない。

#### ナレッジタグとサイドバーのグループ化

`Knowledge.tags`は、ナレッジを分類するための表示用タグ配列である。タグは管理者系ロール（`admin`、`knowledge_manager`）だけがナレッジ作成画面またはナレッジ設定画面で編集できる。タグはナレッジの検索・認可・インタビュー内容を変更せず、左サイドバーの表示グループを構成するために使用する。

Backendは保存前に各タグの前後空白を除去し、空文字を除外し、大文字・小文字を区別せず重複を除去する。重複した場合は最初に指定された表示文字列を保持する。1ナレッジあたりのタグ数は20件以下、1タグの長さは40文字以下とする。制限を超える更新はHTTP 422で拒否する。タグ未設定の既存データは空配列として扱う。入力・選択されたタグはテナント単位の`KnowledgeTag`マスターへ登録し、ナレッジからタグを外してもタグマスターからは削除しない。

ナレッジ作成画面とナレッジ設定画面では、タグマスターと現在取得済みのアクセス可能なナレッジから既存タグの候補リストを作成する。利用者はタグ欄を検索可能なComboboxとして使い、候補から既存タグを選択するか、未登録の文字列を入力して新しいタグを作成できる。タグは1ナレッジにつき1つ選択でき、未設定を選んだ場合はナレッジとの紐付けだけを解除する。候補リストへの追加にも、保存時と同じ正規化ルールを適用する。
...
ナレッジ一覧では、ナレッジ名・タグ・用途・件数・更新日時を列として表示する。業務領域内のナレッジ一覧では、ナレッジ名・タグ・用途・記録数・ドキュメント数・更新日時を列として表示する。タグ未設定の場合は未設定状態を表示し、タグが多い場合はタグ列内でスクロール可能とする。

ナレッジ管理者がナレッジ管理の入口を開いた場合、メイン画面にナレッジ一覧を重複表示してはいけない。最後に開いたナレッジが存在する場合は、そのナレッジの「インタビュー」を開く。最後に開いたナレッジがない場合は、最初のナレッジの「インタビュー」を開く。ナレッジが1件もない場合だけ、メイン画面に作成案内を表示する。`interviewer`は有効なナレッジがある場合に最初のナレッジの「インタビュー」を開き、そこから自分の新しい記録を開始できる。`viewer`はアクセス可能なナレッジがある場合に最初のナレッジの「インタビュー」を開く。アクセス可能なナレッジがない場合は空状態を表示する。

特定のナレッジを開いた画面は、次の2つを主ナビゲーションとして表示する。

| ナレッジ配下の画面 | 役割 |
|---|---|
| インタビュー | `admin`と`knowledge_manager`は新しい記録を作成し、途中の記録を再開する。`interviewer`は有効なナレッジから自分の新しい記録を作成し、自分の途中の記録を再開する。`viewer`は回答操作を行わない。 |
| 記録 | `admin`と`knowledge_manager`はそのナレッジの全記録を一覧表示し、結果の確認・編集・差し戻し・承認を行う。`interviewer`と`viewer`は権限範囲内の記録を確認する。 |

記録詳細のインタビュー画面は、`1200px`以上では左側に会話、右側に会話から整理した情報を表示する。`system_requirement`では、右側の整理結果を「要件整理」と「処理の流れ」の2タブで切り替える。「処理の流れ」タブ内では「フローチャート」と「シーケンス図」を切り替える。`business_process`では右側に処理の流れを表示し、`fixed_form`では右側に質問リストを表示する。`901px`から`1199px`では会話と整理結果の2ペインを維持し、`900px`以下では会話を主領域、整理結果または質問リストを開閉可能な右Drawerへ段階的に縮退する。`767px`以下では会話を1カラムで表示し、回答Composerはdynamic viewportとsafe areaを考慮してソフトウェアキーボードで隠れないようにする。

設定は主ナビゲーションに表示せず、`admin`と`knowledge_manager`だけにナレッジヘッダーの主ボタン「インタビュー設定」を表示する。ナレッジを新規作成した直後は、必ず設定画面の「実行設定」を開く。設定画面では、ナレッジ情報、質問項目、実行設定、事前知識を同じ画面内のタブで管理する。事前知識タブでは、質問設計・インタビュー・回答整理で参照する既知情報と専門用語をテキスト／Markdownで直接入力・編集する。ドキュメント管理を主ナビゲーションの画面として表示してはいけない。`interviewer`と`viewer`にはナレッジ設定、事前知識の管理、ナレッジ作成の操作を表示してはいけない。
...
設定が完了していないナレッジでは、管理者と対象者は記録を作成してはいけない。既存の`draft`記録を回答可能な状態へ変更してはいけない。テキストインタビューの開始、回答送信、音声セッションの開始も許可してはいけない。

Frontendは設定未完了の状態を表示し、管理者系ロールには「設定を開始」を主操作として表示する。対象者には設定変更操作を表示せず、記録作成・開始操作も表示または有効化してはいけない。この状態でBackendが記録作成・開始を受けた場合は、同じ条件を検証して`409 interview_configuration_required`を返す。

### 7.3.2 設定画面の構成

設定画面に「基本設定」タブを置いてはいけない。ナレッジ名と説明は、設定画面上部の「ナレッジ情報」セクションに常に表示する。

「ナレッジ情報」セクションは折りたたみ可能とする。折りたたみ時もナレッジ名を見出しとして表示し、展開時はナレッジ名とタグを横並びにし、説明をその下へ表示する。ナレッジ名を非表示にして、現在の対象を判別できない状態にしてはいけない。
...
| 事前知識 | 既知情報・専門用語のテキスト／Markdown入力、追加・編集・削除、本文表示、保存・検索反映状態とエラー |

「ナレッジ情報」は3つのタブのいずれにも含めず、タブの上に配置する。設定画面の保存操作は、ナレッジ情報、質問項目、実行設定をまとめて保存する。事前知識は各項目の明示的な保存・削除操作で反映し、表示するだけでは更新しない。未保存の編集は識別し、離脱時に警告する。タブごとに別の設定保存状態を持たせてはいけない。
...
### 7.3.3 事前知識の入力とインタビューでの利用

事前知識はKnowledge単位で管理し、登録は任意とする。未登録でもインタビューを開始できる。管理操作は`admin`と`knowledge_manager`に限定し、Backendでテナント・Knowledgeのアクセス権と認証ユーザーを検証する。

入力欄はタイトル、種別、本文で構成する。本文の形式をユーザーに選択させない。既知情報には業務の前提、既存の仕組み、既に分かっている事実を記載する。専門用語には名称、略称・別名、意味、適用範囲を本文として記載できる。本文はプレーンテキストまたはMarkdownをそのまま受け付け、見出しや箇条書きなどの意味のある改行・字下げを保持する。任意HTMLやスクリプトを実行しない。ファイル選択・ドラッグ＆ドロップは提供しない。

本文はBackendで決定的に正規化してから保存・チャンク化する。改行コードをLFへ統一し、外側の空行と空白だけの行を整理し、連続する空行は最大2行までにする。本文中の非空行の字下げ、改行、末尾空白は意味を変えないため保持する。有効なJSONオブジェクトまたは配列だけは構造を変えずに2スペースのインデントへ整形し、JSONとして不正な本文は推測で整形しない。LLMによる書き換えや要約は保存時に実行しない。内部メタデータの`contentFormat`は本文から自動判定する表示・互換用の値であり、入力項目ではない。

Backendは有効な事前知識を質問項目設計、インタビューの質問生成、回答解釈・非同期の回答整理へ渡す。テキスト、Transcribe + Polly、Nova Sonic 2、GPT-Liveで利用方針を共通化する。GPT-Liveにも初期会話コンテキストと回答整理の双方へ渡し、事前知識を使うために独自ターン制御や整理完了待ちを追加してはならない。

* 既知の一般的な前提を最初から説明させず、未確認の内容、差分、具体例を質問する。例えば「承認には製品Aを使用」と登録済みなら、使用製品を聞き直すのではなく、運用上の課題などを聞く。
* 対象者への適用確認や記録上の回答確認が必要な場合は「製品Aをお使いという理解で合っていますか？」のように既知情報を提示して短く確認する。事前知識だけで必須項目を回答済み・確認済みにしない。
* 専門用語・略称は登録された意味を参照して質問と回答を解釈する。文脈で意味を一意に定められないときだけ確認し、用語の説明を毎回求めない。原文を保持し、意味を変える補正には既存の確認ルールを適用する。
* 情報が不足・矛盾している場合は断定せず必要な点だけ確認する。対象者の訂正はその記録の回答として扱い、共有の事前知識をAIが自動で上書きしない。
* 事前知識は参考情報であり、システム指示ではない。本文中の命令で認可、質問進行、承認のルールを変更してはならない。AI提案の正式ナレッジ化には引き続き人の承認を必要とする。

設定として渡す事前知識と、追加の関連情報検索を区別する。`retrievalPolicy=never`は追加検索を禁止する設定であり、明示登録された既知情報・用語の参照まで禁止しない。出典ID・タイトル・改訂識別子を引き継ぎ、既知情報由来の候補と対象者の発言を区別して保存する。根拠と対象項目の対応はBackendが検証する。

入力サイズとモデルのコンテキスト上限を検証し、登録成功した知識を無通知で切り捨てない。全量を渡せない場合は関連箇所を選択し、用語・前提の必要な範囲と出典を渡す。開始時に使用する改訂を固定し、設定変更は次回開始から適用する。削除・無効化された知識は以後の参照から外し、音声モデルへ既に渡したコンテキストの更新を保証できない場合は再接続が必要なことを明示する。

本節は変更後の仕様である。現行実装のAPI・画面・経路は[現行実装](reference/current-implementation.md)を参照する。

### 7.4 記録の担当と状態

各`InterviewRecord`には、回答を担当するインタビュー対象者を1人だけ設定する。担当者は`ownerUserId`に保存する。`ownerUserId`が未設定の記録は、`interviewer`へ表示してはいけない。対象者が自分で新規作成した記録は、Backendが認証ユーザーを`ownerUserId`へ設定し、他ユーザーへの担当者・閲覧者設定を許可してはいけない。
...
`viewer`は会話、音声接続、回答編集、提出、提案承認、文書管理を実行してはいけない。
...
シーケンス図はUMLの基本形式で表示する。参加者を上部の長方形に表示し、参加者ごとに下方向の破線ライフラインを引く。`ProcessInteraction.sequence`の昇順で上から下へメッセージを配置し、送信元から送信先へ向かう横向きの矢印とメッセージ名を表示する。`interactionType`で通常送信、戻り値、非同期処理、通知、引き継ぎ、例外を区別し、`fragmentType`、`fragmentId`、`fragmentLabel`で`alt`、`opt`、`loop`相当の複合フラグメントを表示する。候補状態のメッセージは破線矢印で表示する。シーケンス図は図専用の拡大・縮小、ドラッグ移動、全体表示、リセット操作を持つ。長い参加者名・メッセージ名は表示領域内で折り返し、切り捨てない。シーケンス図の座標とSVG等の表示データはProcessStateからFrontendが生成し、保存しない。
...
管理者系ロールには、全画面下部に要件・処理モデルへの指示入力を表示する。指示はBackendが現在の`RequirementState`と`ProcessState`とともにGPT-5.6 LunaまたはTerraへ渡し、`ProcessModelEditOutput`のStructured Outputとして受け取る。`requirementPatch`は既存RequirementStateの値だけを更新し、`processPatch`はProcessStateの意味構造だけを更新する。条件分岐、例外、権限、対象件数、非同期処理、通知、引き継ぎなどのProcessModel編集指示は、共有するProcessStateを通じてフローチャートとシーケンス図の両方へ反映する。Backendは要件ID、ProcessPatchのバージョン、要素ID、参照関係、`baseStateVersion`を検証してから適用する。要件だけを変更した場合はProcessStateのバージョンを変更しない。管理者編集で変更した要件とProcess要素は`confirmed`として保存するが、Recordの正式承認状態は変更しない。Mermaid、React Flowの座標、HTML、SVG、画像を保存してはならない。指示履歴と変更結果は監査用メッセージおよび監査ログへ保存し、通常チャットにも指示、更新対象、主な反映内容を表示する。
...
質問対象に関連する取り込み済み文書があり、対象項目の値が本文またはタイトルに明示されている場合、Question Generatorはその値を`documentCandidateValue`と根拠の`documentCandidateSourceIds`として返してよい。Backendは返却値が検索済みの文書・チャンクに実在することと、根拠IDが同じテナント・Knowledgeの`indexed`または取り込み完了文書であることを検証する。検証に成功した値は、設備名などの設定済み項目を最初から聞き直す代わりに、文書由来の確認事項として提示する。

事前知識由来の候補は、利用者が明示的に承認するまで正式回答へ保存してはならない。候補を持つ項目は`AWAITING_CONFIRMATION`として保持し、承認時だけ`CONFIRMED`へ移し、訂正時は候補を破棄して利用者発話の値へ置き換える。根拠がない、対象項目との意味的一致が弱い、または有効な参照情報でない場合は候補を作らず、通常の質問を行う。`retrievalPolicy=never`でも7.3.3の設定コンテキストに含まれる既知情報は確認候補に利用できるが、追加検索は行わない。
...
インタビューの次質問生成でも、Backendは同じテナントかつ同じKnowledgeに属する`indexed`または取り込み完了状態の本文・チャンクを共通検索サービスから取得し、Structured InterviewのQuestion Generatorへ`retrieved_knowledge`として渡す。`retrievalPolicy=never`の項目では追加検索せず、`auto`または`required`では質問・項目・会話文脈に関連する情報を検索する。設定として渡す事前知識の扱いは7.3.3に従う。生成された質問には`retrievedSources`として出典ID、タイトル、改訂識別子と検索時のスコアを記録する。`app/voice`は検索・質問進行を実装せず、Backendが生成した同じ質問と出典を利用する。
...
AIインタビューの会話言語はインタビュー開始時に記録単位で選択し、`InterviewRecord.interviewLocale`へ保存する。初期対応は`ja-JP`、`en-US`、`zh-CN`、`pt-BR`とし、テキスト経路と音声経路（Amazon Transcribe + Polly）へ同じ言語設定を渡す。

UI Localeの初期対応は`ja-JP`、`en-US`、`zh-CN`、`th-TH`とする。Localeメタデータは一元管理し、機能単位の翻訳JSONを使用する。Fallbackは`ja-JP`とし、翻訳Key不足・余分なKey・空文字・JSON構文エラーはBuild前の検査で失敗させる。
